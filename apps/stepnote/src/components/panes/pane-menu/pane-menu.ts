...
export class PaneMenu extends LitElement {
  ...
  override render() {
    ...
    return html`
      <!-- 上部エリア (メイン機能群) -->
      <div class="pane-menu__primary">
        <wa-tooltip for="pane-menu-toggle-btn" placement="right">
          ${toggleTooltip}
        </wa-tooltip>
        <wa-button
          id="pane-menu-toggle-btn"
          class="pane-menu__toggle-btn"
          variant="neutral"
          appearance="plain"
          size="s"
        ...
        >
          <wa-icon
            library="my-icons"
            name="chevron-right"
            class="pane-menu__toggle-icon ${isOpen ? "is-open" : "is-closed"}"
            label=${toggleTooltip}
          ></wa-icon>
        </wa-button>
      </div>
      ...
      <!-- 下部エリア (将来の補助機能群) -->
      <div class="pane-menu__secondary"></div>
    `;
  }
}
