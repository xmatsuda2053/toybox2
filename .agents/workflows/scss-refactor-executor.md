...
8. **コンポーネント同時同期と旧クラス併記放置の禁止（完全移行の徹底）**:
   - SCSS のセレクタ名を変更（簡易BEM移行等）する場合、後方互換を目的とした旧セレクタの併記放置を厳禁とする。
   - 影響を受けるコンポーネント（TypeScript / Lit テンプレート）側のクラス名も同一作業内で同時に修正し、新命名規則へ完全一本化すること。
   - セレクタ名変更に伴い、単体テスト（`*.test.ts`）内のクラス検証アサーションを最新化し、旧クラス名が一切残存していないことを保証するアサーションを追加すること。
...
1. 対象コンポーネント（`*.ts` / `*.scss` / `*.test.ts`）を特定。
2. **テスト先行更新（🔴 Red）**: 既存の旧クラス名検証を新クラスへ更新し、旧クラス名が一切残存していないことを検証するテストケースを追加して失敗（Red）を確認・ユーザー報告。
3. **同時同期実装（🟢 Green）**:
   - TypeScript（Lit テンプレート）のクラス名を新規則（簡易BEM等）へ改名。
   - ペア SCSS のセレクタを新規則のみへ一本化（旧セレクタの併記は行わない）。
4. **1コンポーネント単位の確認停止**: 複数コンポーネントを同一ブランチで扱う場合であっても、1つのコンポーネントの修正完了ごとにユーザーへ確認を求め、承認を得てから次のコンポーネントへ進む。
5. 開発サーバー（`npm run dev:stepnote`）やビルド成果物で画面を目視確認し、レイアウト崩れがないことを検証。
